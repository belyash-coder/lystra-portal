...
export function ReviewComments({ reviewId, initialCommentsCount = 0, children }: ReviewCommentsProps) {
    ...
    if (reviewIdFromUrl === reviewId) {
      ...
      if (comments.length === 0) {
        const fetchCommentsFromUrl = async () => {
          setIsFetching(true);
          // TODO: Заменить на fetch('/api/comments?reviewId=...')
          // Временно отключено
          setIsFetching(false);
        };
  ...
  }, [searchParams, reviewId, comments.length]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // TODO: Переписать на fetch('/api/comments', { method: 'POST', body: ... })
      
      // Временная заглушка-имитация отправки комментария
      const dummyComment: Comment = {
        ...
        profiles: {
          username: 'Вы', // Здесь будет имя текущего юзера
          avatar_url: null
        }
      };
  ...
  const handleReplyClick = (commentId: string, username: string, explicitRootId: string | null) => {
    setReplyingTo({ 
      id: commentId, 
      username, 
      rootId: explicitRootId || commentId
    });
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <button 
                        ...
                        className="text-[10px] text-neutral-500 hover:text-[#a78bfa] font-medium mt-1.5 ml-2 transition-colors cursor-pointer"
                      >
                        Ответить
                      </button>
                    </div>
                        ...
                        <div key={reply.id} className="flex gap-3 ml-9">
                          ...
                          <div className="flex-1">
                            ...
                            <button 
                              ...
                              className="text-[10px] text-neutral-500 hover:text-[#a78bfa] font-medium mt-1 ml-2 transition-colors cursor-pointer"
                            >
                              Ответить
                            </button>
                          </div>
          ...
          <div className="relative">
            {replyingTo && (
              <div className="flex items-center justify-between bg-neutral-800/50 rounded-t-lg px-3 py-1.5 border border-b-0 border-neutral-700/50 mb-[-1px] text-[10px]">
                ...
                <button onClick={() => setReplyingTo(null)} className="text-neutral-500 hover:text-rose-400 transition-colors p-0.5 cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              </div>
            )}
            <form onSubmit={handleSubmit} className="flex gap-2 relative">
              <input
                ...
                placeholder="Временно отключено (Офлайн)"
                disabled={true} 
                className={`w-full bg-[#1a1a1a] border border-neutral-800 pl-4 pr-10 py-2.5 text-xs text-white focus:outline-none focus:border-[#a78bfa] transition-colors disabled:opacity-50 cursor-not-allowed ${
              ...
              <button
                type="submit"
                disabled={true}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-neutral-500 hover:text-[#a78bfa] disabled:opacity-50 disabled:hover:text-neutral-500 transition-colors cursor-not-allowed"
              >
